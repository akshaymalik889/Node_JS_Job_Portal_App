...
export const createJob = TryCatch(async (req, res) => {
    ...
    const [newJob] = await sql `INSERT INTO jobs (title, description, salary, location, role, job_type, work_location, 
...
export const updateApplication = TryCatch(async (req, res) => {
    ...
    const [updatedApplication] = await sql `UPDATE applications SET status = ${req.body.status} WHERE application_id = ${id} 
      RETURNING *`;
    const message = {
