...
export const deleteCompany = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const [company] =
      await sql`SELECT logo_public_id FROM companies WHERE company_id = ${companyId} 
...
export const createJob = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  const [company] =
    await sql`SELECT company_id FROM companies 
  ...
  const [newJob] =
    await sql`INSERT INTO jobs (title, description, salary, location, role, job_type, work_location, 
    company_id, posted_by_recuriter_id, openings) VALUES (${title}, ${description}, ${salary}, ${location}, 
...
export const getAllActiveJobs = TryCatch(async (req, res) => {
  ...
  let querySting = `SELECT j.job_id, j.title, j.description, j.salary, j.location, j.job_type, 
  j.role, j.work_location, j.created_at, c.name AS company_name, c.logo AS company_logo, 
  c.company_id AS company_id FROM jobs j JOIN companies c ON j.company_id = c.company_id 
...
export const updateApplication = TryCatch(
  async (req: AuthenticatedRequest, res) => {
    ...
    const [updatedApplication] =
      await sql`UPDATE applications SET status = ${req.body.status} WHERE application_id = ${id} 
      RETURNING *`;
